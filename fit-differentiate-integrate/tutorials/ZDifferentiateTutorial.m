...
% numerical time differentiation can be performed to give an 
...
% How accurate was that estimation of the v(t) function? It is 
% almost impossible to know exactly at this point. 
...
% Supposedly one also wants to find the values of the 
% acceleration function a(t) in the x coordinate every 0.5 second 
% in the time interval [-1 s, 9 s]. 
% To find the acceleration in the x coordinate, the second-order 
% time derivative of the position in the x coordinate would have 
% to be calculated. The second- and higher-order numerical 
...
% the optional parameter named ''OrdDeriv'', the default value of 
% which is 'OrdDeriv' = 1. 
% The actual acceleration function a(t) is a(t) = -sin(t) if time t is 
% expressed in seconds and the acceleration a in the x 
% coordinate is expressed in meters per seconds squared (i.e. 
% m / (s^2)). 
% Run the following to block to compare the actual acceleration 
% function a(t) to the estimated values of the a(t) function in the 
...
% optional parameter named ''Accuracy'', the default value of 
% which is 'Accuracy' = 2. 
...
xDerivative = (-1 : 0.5 : 9)';   % Time points at which a(t) is to be 
                                               % evaluated. 
OrdDeriv = 2;   % Order of differentiation. 
Accuracy = 2;   % Accuracy. Set the value of this parameter to a 
                          % different natural number to see its effect on the 
                          % a(t) estimation.
yDerivative = ZFindDerivative(xData, yData, ...
    xDerivative, 'OrdDeriv', OrdDeriv, ...
    'Accuracy', Accuracy);   % a(t) estimation. 
TT = (linspace(-1, 9, 1000))';
aActual = -sin(TT);   % Actual a(t). 
close all; figure(1); hold on;
plot(TT, aActual, 'k', 'LineWidth', 1.5);
...
% parameter named ''Mode'', the default value of which is 
% 'Mode' = 1 but can also be set to either 0 or 2. To understand 
% the three modes in detail, documentation and the 
% code of the functions and their subfunctions should be 
% referred to. 
...
yDerivative0 = ZFindDerivative(xData, yData, xDerivative, ...
     'Accuracy', Accuracy, 'Mode', 0);   % yDerivative estimate if 
                                                               % 'Mode' paramater is 0. 
yDerivative1 = ZFindDerivative(xData, yData, xDerivative, ...
     'Accuracy', Accuracy, 'Mode', 1);   % yDerivative estimate if 
                                                               % 'Mode' paramater is 1. 
yDerivative2 = ZFindDerivative(xData, yData, xDerivative, ...
     'Accuracy', Accuracy, 'Mode', 2);   % yDerivative estimate if 
                                                               % 'Mode' paramater is 2. 
close all; figure(1); hold on;
...
% points. This can be done by setting the optional parameter 
% named ''Type'' to ''Spline''. 
...
% the velocity function v(t) over the time interval [-1 s, 9 s]. 
...
% needs to be set. The 'PolyDegree' parameter is equal to the 
% degree of the regression polynomial. 
% As with the ''Spline'' setting of the 'Type' parameter, there are 
% no optional parameters 'Accuracy' and 'Mode'. 
...
% local extremes judging by the data points. 
...
% cases, the optional parameter named ''Figure'' can be set as 
% the index of the figure window in which the function is to be 
% visualized. 
