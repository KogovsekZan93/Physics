function [yDerivativeSpline, varargout] = ZFindDerivativeSplineBasic...
...
% ZFindDerivativeSplineBasic(xData, yData, xDerivativeSpline, varargin)
% 
...
%    -'ppDerivativeSpline' is the piecewise polynomial which is the 
%     estimation of the function f^('OrdDeriv'). It can be evaluated by 
%     the MATLAB ppval function. The details of the 
%    -'ppDerivativeSpline' piecewise polynomial can be extracted by the 
%     MATLAB unmkpp function. 
%    -'ppFitSpline' is the interpolating cubic spline of the data points 
%     represented by the pairs ('xData'(i), 'yData'(i)) which is the 
%     estimation of the f function. It can be evaluated by the MATLAB 
%     ppval function. The details of the 'ppFitSpline' piecewise 
...
% In the following block of code, the coefficients of the differentiated 
% polynomials of the piecewise polynomial which is the estimated 
% f^('OrdDeriv') function are calculated. The differentiated polynomials 
% are appropriate derivatives of the polynomials of which the cubic spline 
% which interpolates the data point is constructed. 
CoefsDeriv = Coefs;
if OrdDeriv < 4
    for i = 1 : OrdDeriv
...
% In the following line, the piecewise polynomial which represents the 
% f^('OrdDeriv') function is constructed using the MATLAB mkpp function. 
...
% f^('OrdDeriv') is evaluated for the values of the 'xDerivativeSpline' 
