...
%% Numerical piecewise interpolation polynomial-based differentiation tool
% 
% Author: Žan Kogovšek
% Date: 1.3.2023
...
% 'yDerivativeA' of the estimated values of 
% f^('OrdDeriv')('xDerivativeA'), where f^('OrdDeriv') is the 
% 'OrdDeriv'-th order derivative of the f function and 'xDerivativeA' is 
% the input vector of values of the X variable. The estimation is based on 
% a piecewise interpolation polynomial of the data points represented by 
...
% at which the value of the derivative of the f function is to be 
% estimated. The 'xDerivativeA' vector must be a column vector of real 
% numbers. The values of the 'xDerivativeA' vector must be in ascending 
% order. 
% 
% 'varargin' represents the optional input parameters named ''OrdDeriv'', 
% ''Accuracy'', and ''Mode''. 
...
%     1. 
%    -''Accuracy'' is the name of the parameter the value of which is the 
%     order of the interpolation polynomials from which the piecewise 
%     interpolation polynomial which represents the f function is to be 
%     composed. It must be a nonnegative integer. The default value is 0. 
...
%     piecewise interpolation polynomial which represents the f function. 
%     It must be one of the three integers: 0, 1, or 2. The default value 
%     is 0. 
% 
% 'yDerivativeA' is the column vector of the estimated values of 
% f^('OrdDeriv')('xDerivativeA'). 
% 
% 'varargout' represents the optional output parameters 'Ipoints' and 
% 'Smatrix'. 
%    -'Ipoints' is a column vector of boundaries between the interpolation 
%     polynomials of the piecewise interpolation polynomial which is the 
%     estimation of the f function. Any two consecutive values of the 
%     'Ipoints' vector 'Ipoints'(i) and 'Ipoints'(i + 1) are the 
%     boundaries of i-th interpolation polynomial. 
%    -'Smatrix' is the matrix of rows of indices. Each row 'Smatrix'(i, :) 
%     contains the indeces k of the data points ('xData'(k), 'yData'(k)) 
%     which were used to construct the i-th interpolation polynomial of 
%     the piecewise interpolation polynomial which is the estimation of 
%     the f function. 
%     The piecewise polynomial which is used to estimate the f function 
%     can be evaluated by using the parameters 'Ipoints' and 'Smatrix' as 
%     the input of the EvaluateIpointsSmatrixFit function. 
...
% In the following line, the EvaluateIpointsSmatrixDerivative function is 
% used to estimate the values of the f^('OrdDeriv')('xDerivativeA') vector 
% by calculating the derivative of the piecewise interpolation polynomial 
% represented by the variables 'Ipoints' and 'Smatrix' for the values of 
% the 'xDerivativeA' vector. 
