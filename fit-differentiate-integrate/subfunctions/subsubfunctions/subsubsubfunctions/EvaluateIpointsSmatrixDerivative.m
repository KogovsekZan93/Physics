...
% 'yDerivativeA' of the values of fA^('OrdDeriv')('xDerivativeA'), 
% where fA^('OrdDeriv') is the 'OrdDeriv'-th order derivative of 
% the fA function and 'xDerivativeA' is the input vector of values 
% of the X variable. The fA function itself is the input 'Ipoints' 
% vector- and the input 'Smatrix' matrix-defined piecewise 
...
% variable X at which the yDerivativeA vector values of the 
% fA^('OrdDeriv') function are to be calculated. The 
% fA^('OrdDeriv') function is the 'OrdDeriv'-th derivative of the 
% piecewise interpolation polynomial fA of the data points 
% represented by the pairs ('xData'(i), 'yData'(i)) and is defined 
% by the input variables the 'Ipoints' vector and the 'Smatrix' 
% matrix. Each polynomial p_i of which the fA function consists 
% is the interpolation polynomial of the points 
...
% 'yDerivative' vector will be that of the derivative of the 
% polynomial p_i for the 'xDerivativeA'(k) value (in other words, 
% the polynomial p_i on the right-side of the boundary is used 
% for the calculation of 'yDerivativeA'(k) value). 
...
% natural number. 
% 
% 'Ipoints' is the column vector of boundaries between the 
...
% ('xData'(k), 'yData'(k)) which are used to construct the i-th 
...
% 'xDerivativeA'(1) is contained is found. This way, useless 
% calculation of the coefficients of the derivatives of the 
% interpolation polynomials p_J for J < 'j' is avoided. 
j = 2;
while Ipoints(j) <= xDerivativeA(1)
    j = j +1;
end

% For each relevant interval of the X variable from 'Ipoints'('j' - 1) 
% to 'Ipoints'('j'), firstly the coefficients vector 'pDerivativeA' of 
% the polynomial which is the 'OrdDeriv'-th derivative of the 
% interpolation polynomial p_('j' - 1) of the data points 
