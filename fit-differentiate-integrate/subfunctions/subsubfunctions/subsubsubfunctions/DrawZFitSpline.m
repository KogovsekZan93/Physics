...
%% Tool for plotting the data points and the interpolating spline curve
% 
% Author: Žan Kogovšek
% Date: 4.22.2023
...
% arbitrary function Y = f(X), the piecewise polynomial structure 
% 'ppFitSpline' of the interpolating cubic spline polynomial fSpline of 
% the data points represented by the pairs ('xData'(i), 'yData'(i)), and 
% the natural number 'Figure', this function plots the data points and the 
% spline curve of the data points from the input value of the X variable 
% 'xFitSplineMin' to the input value of the X variable 'xFitSplineMax'. 
% 
%% Variables
% 
% This function has the form of DrawZFitSpline...
% (Figure, xData, yData, xFitSplineMin, xFitSplineMax, ppFitSpline)
% 
% 'Figure' is the parameter the value of which is the index of the figure 
% window on which the data points and the spline curve described in the 
...
% 
% The 'xFitSplineMin' parameter and the 'xFitSplineMax' parameter are two 
% values of the X variable and are the lower and the upper boundary, 
% respectively, of the interval over which the cubic spline curve of the 
% data points represented by the pairs ('xData'(i), 'yData'(i)) is to be 
% plotted. The 'xFitSplineMax' value must be greater than the 
% 'xFitSplineMin' value. 
% 
...
% points for the interpolating spline curve plot. With this setting, the 
% number of points is typically sufficient to create a convincing illusion 
% of the plotted fSpline function curve being smooth (as the actual 
% fSpline function is, in fact, a smooth function). 
N = power(10, 4);
