function DrawZIntegralA(Figure, xData, yData, ...
    xIntegralAMin, xIntegralAMax, ColorFace, Ipoints, Smatrix)
%% Tool for plotting the data points, the piecewise interpolation 
%% polynomial curve, and the area under the curve over an interval
% 
% Author: Žan Kogovšek
% Date: 3.24.2023
...
% arbitrary function Y = f(X), as well as the vector 'Ipoints' and the 
% matrix 'Smatrix', both of which define the fA piecewise interpolation 
% polynomial of the data points represented by the pairs 
% ('xData'(i), 'yData'(i)), the values of the X variable the 
% 'xIntegralAMin' value and the 'xIntegralAMax' value, the natural number 
% 'Figure', and the vector 'ColorFace', this function plots the data 
% points, the piecewise interpolation polynomial curve of the data points, 
% and the area under the piecewise interpolation polynomial curve from 
% 'xIntegralAMin' to 'xIntegralAMax', the color of the area being defined 
% by the RGB triplet of numbers of the 'ColorFace' vector. 
% 
%% Variables
% 
% This function has the form of DrawZIntegralA(Figure, xData, yData, ...
% xIntegralAMin, xIntegralAMax, ColorFace, Ipoints, Smatrix)
% 
% 'Figure' is the parameter the value of which is the index of the figure 
% window on which the data points, the piecewise interpolation polynomial 
% curve, and the area under the curve described in the Description section 
% is to be plotted. The value of the 'Figure' parameter must be a natural 
% number. 
% 
...
% 
% The 'xIntegralAMin' parameter and the 'xIntegralAMax' parameter are two 
% values of the X variable and are the lower and the upper boundary, 
% respectively, of the area to be plotted using this function under the 
% piecewise interpolation polynomial curve of the data points represented 
% by the pairs ('xData'(i), 'yData'(i)). The 'xIntegralAMax' value must be 
...
% area under the piecewise interpolation polynomial curve of the data 
% points represented by the pairs ('xData'(i), 'yData'(i)) from the value 
% 'xIntegralAMin' to the 'xIntegralAMax' value. The three real numbers 
% must be values of the [0, 1] interval. 
% 
...
% were used to construct the i-th interpolation polynomial p_i of the 
...
DrawZAreaA(xData, yData, xIntegralAMin, xIntegralAMax, ColorFace, ...
    Ipoints, Smatrix)

hold on;

% In the following block of code, the piecewise interpolation polynomial 
% curve of the data points represented by the pairs 
% ('xData'(i), 'yData'(i)) as well as the data points themselves are 
% plotted. 
