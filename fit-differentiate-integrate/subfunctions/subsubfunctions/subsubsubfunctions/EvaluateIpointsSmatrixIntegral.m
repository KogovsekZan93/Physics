...
% defined from 'Ipoints'(i) to 'Ipoints'(i + 1). 'Smatrix' is the input 
% matrix of indices and 'Ipoints' is the input vector of values of 
% the X variable. 
% 
%% Variables
% 
...
% boundary 'j' of the interval ('Ipoints'('j' - 1), 'Ipoints'('j')) in which 
% the lowest value of the 'xIntegralA' vector (i.e. the 
% 'xIntegralA'(1) value) is found. This is how the first relevant 
% interval ('Ipoints'('j' - 1), 'Ipoints'('j')) and the 
% polynomial p_('j' - 1) are determined so no further 
% consideration is required for the intervals 
% ('Ipoints'(J - 1), 'Ipoints'(J)) and the corresponding 
% polynomials p_(J - 1) for J < 'j'. 
j = 2;
...
% value to both the boundaries of the interpolation 
% polynomials p_i and the values of the 'xIntegralA' vector. 
Summa = 0;
a = 1;

% For each relevant interpolation polynomial p_('j' - 1), the 
% vector 'pIntegralA' of the coefficients of the polynomial 
...
% pIntA_('j' - 1), the 'yIntegralA' vector is evaluated for the values 
% of the 'xIntegralA' vector within the interval over which the 
% relevant pIntA_('j' - 1) polynomial is defined. Also, the 
% 'Summa' parameter is increased appropriately at that time to 
% account for the definite integral of the dfA/dX function from the 
% value 'xIntegralA'('b' - 1) to the value 'xIntegralA'('b'). 
