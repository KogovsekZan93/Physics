function [yDerivativePolyFit, varargout] = ZFindDerivativePolyFitBasic...
    (xData, yData, xDerivativePolyFit, PolyDegree, varargin)
%% Numerical polynomial regression-based differentiation tool
% 
% Author: Žan Kogovšek
% Date: 1.1.2023
...
% 'yDerivativePolyFit' of the estimated values of 
% f^('OrdDeriv')('xDerivativePolyFit'), where f^('OrdDeriv') is the 
% 'OrdDeriv'-th order derivative of the f function and 
% 'xDerivativePolyFit' is the input vector of values of the X variable. 
% The estimation is based on the polynomial regression of the data points 
...
% 
% 'xDerivativePolyFit' is the vector of the values of the independent 
% variable X at which the value of the derivative of the f function is to 
% be estimated. The 'xDerivativePolyFit' vector must be a column vector of 
% real numbers. 
% 
...
% nonnegative integer. 
% 
% 'varargin' represents the optional input parameter named ''OrdDeriv''. 
% ''OrdDeriv'' is the name of the parameter the value of which is the 
% derivative order. It must be a natural number. The default value is 1. 
% 
% 'yDerivativePolyFit' is the column vector of the estimated values of 
% f^('OrdDeriv')('xDerivativePolyFit'). 
% 
% 'varargout' represents the optional output parameters. The optional 
% parameters are 'pDerivativePolyFit' and 'pFitPolyFit'. 
%    -'pDerivativePolyFit' is the vector of coefficients of the polynomial 
%     which is the 'OrdDeriv'-th derivative of the 'PolyDegree'-th degree 
%     regression polynomial of the data points represented by the pairs 
%     ('xData'(i), 'yData'(i)). 
%     'pDerivativePolyFit' is a column vector of the form of 
%     [a_('PolyDegree' - 'OrdDeriv'); 
%     a_('PolyDegree' - 'OrdDeriv' - 1); ..., a_1; a_0]. The 
%     differentiated regression polynomial can be evaluated by the MATLAB 
%     polyval function. 
%    -'pFitPolyFit' is the vector of coefficients of the 'PolyDegree'-th 
%     degree regression polynomial of the data points represented by the 
%     pairs ('xData'(i), 'yData'(i)). 
%     'pFitPolyFit' is a column vector of the form of 
%     [a_'PolyDegree'; a_('PolyDegree' - 1); ...; a_1; a_0]. The 
%     regression polynomial can be evaluated by the MATLAB polyval 
%     function. 
...
% In the following two lines, in the first line, the regression polynomial 
% coefficients of the data points which are represented by the pairs 
% ('xData'(i). 'yData'(i)) are calculated. In the second line, the 
% coefficients of the polynomial which is the derivative of the 
% regression polynomial are calculated. 
pFitPolyFit = (polyfit(xData, yData, PolyDegree))';
pDerivativePolyFit = CalculateDerivativePolynomialCoefficients...
