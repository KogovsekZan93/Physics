...
%% Numerical piecewise interpolation polynomial-based definite integration 
...
% 'DefiniteIntegralA' of the definite integral 
% f('Limits'(2)) - f('Limits'(1)), where 'Limits' is the input vector of a 
% pair of values of the X variable. The estimation is based on a piecewise 
...
%     'Limits'(1) > 'Limits'(2) and blue if not. 
...
% 
% 'DefiniteIntegralA' is the estimated value of the integral of the df/dX 
% function over the X variable with the lower limit 'Limits'(1) and the 
...
% 'DefiniteIntegralA' is calculated by multiplying the 'yIntegralA'(2) 
% value by either 1 or -1 based on the order of the limits on integration. 
DefiniteIntegralA = yIntegralA(2) * LimitOrder;

DrawZIntegralAHandle = @DrawZIntegralA;
DrawZIntegralAInput = {xData, yData, LimitsSorted(1), LimitsSorted(2), ...
