...
%% Tool for finding the average values and the covariance 
%% matrix of the output variables
% 
% Author: Žan Kogovšek
% Date: 9.17.2023
% Last changed: 11.21.2023
% 
%% Description
% 
...
% integer 'N_Rnd', this function returns the vector 'Avg_f' and the 
% 'CovarMat_f' matrix. The 'Avg_f'(j) value is the average value 
% of the j-th output variable of the f function and the 
% 'CovarMat_f'(j, jj) value is the value of the covariance of the j-th 
% and the jj-th output variable of the f function. 
% The distributions of the individual output variables can also be 
% plotted along with the normal distribution curves which have 
% the same averages and standard deviations as the output 
% variables. 
% The values of the 'Avg_f' vector and the 'CovarMat_f' matrix 
% are based on 'N_Rnd' valid outputs of the f function, where 
% the input values are randomly generated based on the 
...
%         index of the figure window on which the distribution of the 
%         vec_f(m) variable is to be plotted (if 
%         vec_F(m + length_f) = 0, the distribution of the vec_f(m) 
%         variable will not be plotted). 
...
%         vector are valid. The values of the vec_f vector will only 
%         be used in the calculation of the average and the 
...
% average and the covariance of each variable of the 
% output vector of the f function. It must be a natural number. 
% 
% 'Avg _f' is a column vector and 'CovarMat_f' is a matrix. The 
% 'Avg_f'(j) value is the average value of the j-th output variable 
% of the f function and the 'CovarMat_f'(j, jj) value is the value of 
% the covariance of the j-th and the jj-th output variable of the f 
% function. 
% The values of the 'Avg _f' vector and the 'CovarMat_f' matrix 
% are based on 'N_Rnd' valid outputs of the f function, the 
% validity of the outputs being based on the value of the final 
% output variable of the F function. The input values for each 
% valid output of the f function are randomly generated based on 
% the distribution of the input variables, specified by the 
...
% which has the same average and standard deviation as the 
% distribution of the j-th output variable is plotted. 
