...
% time t, such as velocity V in the X direction. A value of V is 
...
% t == 0 is x == 0 and wishes to find the value of the X 
% coordinate at t === 8 s. In order to do that, one has to 
% integrate the velocity function V(t). Only the values in nine 
% specific time points have been measured and the velocity 
% function as a whole is unknown. In such cases, numerical 
% integration has to be performed to give an accurate estimation 
% of the actual value of the X coordinate. ZIntegralA is a tool 
% which can be used to do just that. The simplest way of 
% integrating a function numerically can be performed and 
% visualized using the ZIntegralA function by setting the 
% pseudo-order of accuracy to 0. Run the following block of 
